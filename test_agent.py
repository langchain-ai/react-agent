...
async def main():
    ...
    print("Sending Airflow Log to agent with Gemini 1.5 Flash...")
    try:
        async for chunk in graph.astream(
            inputs,
            stream_mode="values",
            # context=Context(model="ollama/qwen2.5-coder:7b"),
            context=Context(model="google_genai/gemini-3-flash-preview"),
        ):
